...
{
   public:
    SimpleVector() : m_size(0), m_begin(nullptr){};
...
class MeshElementBase  // prototype of vertices, edges and faces
{
   public:
    ...
    MeshElementBase() : m_id(0), m_type(UNDEFINED_POINT){};
...
class Vertex : public MeshElementBase, public Point3D {
   public:
    enum VertexState { OUTSIDE, FRONT, INSIDE };

    Vertex() {
        m_type              = VERTEX;
        m_geodesic_distance = geodesic_VTP::GEODESIC_INF;  // Initialize
                                                           // Distance
        m_state              = OUTSIDE;
    ...
    bool operator()(vertex_pointer const& x,
                    vertex_pointer const& y) const  // compare geodesic
    ...
    };  // return whether the vertex is saddle vertex or boundary vertex

    face_pointer& incident_face() {
        return m_incident_face;
    };  // return the incident face of shortest path to the vertex
    double& incident_point() {
        return m_incident_point;
    };  // return the incident point position of the shortest path

    VertexState& state() {
        return m_state;
    };  // return the state of vertex according to traversed area R
    double& geodesic_distance() {
        return m_geodesic_distance;
    };  // return the geodesic distance of the vertex

   private:
    // member
    bool m_saddle_or_boundary;  // it is true if total adjacent angle is larger
                                // than 2*PI or this vertex belongs to the mesh
                                // boundary

    face_pointer m_incident_face;  // the incident face of shortest path to the
                                   // vertex
    double m_incident_point;  // the incident point position of the shortest
                              // path
...
class Edge : public MeshElementBase {
   public:
    ...
    face_pointer opposite_face(face_pointer f) {
        ...
        return adjacent_faces()[0]->id() == f->id() ? adjacent_faces()[1] :
                                                      adjacent_faces()[0];
    };
...
class SurfacePoint : public Point3D  // point on the surface of the mesh
{
   public:
    ...
    SurfacePoint(edge_pointer e,  // set the surface point in the middle of the
                                  // edge
                 double a = 0.5)
        : m_p(e) {
        double b = 1 - a;
