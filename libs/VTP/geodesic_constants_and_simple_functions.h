...
// statistics
static double   m_time_consumed;  // how much time does the propagation step takes
static unsigned m_queue_max_size;       // used for statistics
static unsigned m_windows_propagation;  // how many time a window is propagated
static unsigned m_windows_wavefront;  // the number of windows on the wavefront
static unsigned m_windows_peak;       // the maximum number of windows, used to
                                      // calculate the memory

// two windows' states after checking
enum windows_state { w1_invalid, w2_invalid, both_valid };

inline double cos_from_edges(double const a,  // compute the cosine of the angle
                                              // given the lengths of the edges
...
inline double angle_from_edges(double const a,  // compute the cosine of the
                                                // angle given the lengths of the
                                                // edges
                               double const b, double const c) {
...
inline bool read_mesh_from_file(char* filename, Points& points, Faces& faces,
    ...
    while (getline(file, curLine)) {
        ...
        if (curLine[0] == 'v' && curLine[1] != 't') {
            std::map<std::string, int>::iterator pos = mapForDuplicate.find(
                curLine);
            if (pos == mapForDuplicate.end()) {
                int oldSize = mapForDuplicate.size();
                realIndex.push_back(oldSize);
                mapForDuplicate[curLine] = oldSize;
                //                                sscanf_s(curLine.c_str(), "v
                //                                %lf %lf %lf", &coord[0],
                //                                &coord[1], &coord[2]);
                sscanf(curLine.c_str(), "v %lf %lf %lf", &coord[0], &coord[1],
                       &coord[2]);
            ...
            if (curLine.find('/') != std::string::npos)
                //                              sscanf_s(curLine.c_str(), "f
                //                              %d/%d %d/%d %d/%d", &vtxIdx[0],
                //                              &tex, &vtxIdx[1], &tex,
                //                              &vtxIdx[2], &tex);
                sscanf(curLine.c_str(), "f %d/%d %d/%d %d/%d", &vtxIdx[0], &tex,
                       &vtxIdx[1], &tex, &vtxIdx[2], &tex);
            else
                //                              sscanf_s(curLine.c_str(), "f %d
                //                              %d %d", &vtxIdx[0], &vtxIdx[1],
                //                              &vtxIdx[2]);
                sscanf(curLine.c_str(), "f %d %d %d", &vtxIdx[0], &vtxIdx[1],
                       &vtxIdx[2]);

            vtxIdx[0] = realIndex[vtxIdx[0] - 1];
            vtxIdx[1] = realIndex[vtxIdx[1] - 1];
            vtxIdx[2] = realIndex[vtxIdx[2] - 1];
            if (vtxIdx[0] == vtxIdx[1] || vtxIdx[0] == vtxIdx[2] ||
                vtxIdx[1] == vtxIdx[2])
                continue;
...
inline void CalculateIntersectionPoint(double X1,
                                       double Y1,  // compute intersection point
                                                   // of two windows
...
inline bool PointInTriangle(double& X,
                            double& Y,  // judge if a point is inside a triangle
