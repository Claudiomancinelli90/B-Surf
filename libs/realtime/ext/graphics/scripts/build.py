...
@cli.command()
def release():
    os.system('mkdir -p build && mkdir -p build/release && cd build/release && cmake ../.. -GNinja && ninja')
...
@cli.command()
def debug():
    os.system('mkdir -p build && mkdir -p build/debug && cd build/debug && cmake ../.. -GNinja -DCMAKE_BUILD_TYPE=Debug && ninja')

@cli.command()
def xcode():
    os.system('mkdir -p build && mkdir -p build/xcode && cd build/xcode && cmake ../.. -GXcode && open my-opengl.xcodeproj')
