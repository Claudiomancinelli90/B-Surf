...
static std::unordered_map<vec2i, int> add_vertices(
...
static bool subdiv_shape(vector<vec3i> &triangles, vector<vec3f> &positions,
    vector<int> &parent_faces, float threshold) {
  auto new_vertices = add_vertices(triangles, positions, threshold);
...
static vector<int> subdiv_shape(
    vector<vec3i> &triangles, vector<vec3f> &positions, float threshold) {
  auto parent_faces = vector<int>{};
  while (subdiv_shape(triangles, positions, parent_faces, threshold)) {
  }
  return parent_faces;
}
...
dual_geodesic_solver make_dual_geodesic_solver(const vector<vec3i> &_triangles,
    const vector<vec3f> &_positions, const vector<vec3i> &_adjacencies) {
  auto get_triangle_center = [](const vector<vec3i>  &triangles,
                                 const vector<vec3f> &positions,
                                 int                  face) -> vec3f {
    auto  tr     = triangles[face];
    vec3f pos[3] = {positions[tr.x], positions[tr.y], positions[tr.z]};
    ...
    return (positions[tr.x] + positions[tr.y] + positions[tr.z]) / 3;
  };
  ...
  // solver.parent_faces = subdiv_shape(triangles, positions, 0.03);
...
geodesic_solver make_geodesic_solver(const vector<vec3i> &triangles,
  ...
  for (auto i = 0; i < positions.size(); ++i) {
    ...
    for (auto j = 0; j < star.size(); ++j) {
      ...
      auto k     = find_in_vec(adjacencies[tid],
              opp);  // TODO(fabio): this is not needeed
      assert(k != -1);
...
// TODO: cleanup
// TO DO (claudio):the connectivity of a mesh_point changed again,modify this
...
static vector<float> funnel(
  ...
  for (auto i = 0; i < points.size() - 1; i++) {
    ...
    for (auto k = points[i].face; k < points[i + 1].face; k++) {
      ...
      lerps.push_back(clamp(s, 0.0f, 1.0f));
    }
  }
